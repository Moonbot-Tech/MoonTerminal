...
pub struct Channels {
    ...
    /// See `crate::hl_diag`.
    pub hl_limit: bool,
    /// ClientSettings snapshot: the global coin blacklist and every temporary-blacklist row the
    /// core holds, in the spelling the wire uses. See `crate::settings_diag`.
    pub settings: bool,
...
pub fn apply_env(cfg: &mut DiagCfg, get: impl Fn(&str) -> Option<String>) {
    ...
    on("MOON_SETTINGS_DIAG", &mut cfg.channels.settings);
