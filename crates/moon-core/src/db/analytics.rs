...
/// Сводка за период `[from, to)` (unix-сек UTC). `from < 0` — вся история
/// (от MIN(closedate)). None — БД/таблицы ещё нет.
pub fn summary(from: i64, to: i64) -> Option<Summary> {
    ...
    let (cur, days, hours) = scan_period(&conn, from, to, bucket);
    let (prev, _, _) = scan_period(&conn, from - len, from, bucket);
    ...
    Some(Summary {
        ...
        bucket_secs: bucket,
        days,
        best: top_trades(&conn, from, to, has_strat_names, true),
        worst: top_trades(&conn, from, to, has_strat_names, false),
        strategies: groups(&conn, from, to, has_strat_names, true),
        coins: groups(&conn, from, to, has_strat_names, false),
        best_hour,
        from,
        to,
    })
...
fn scan_period(
    ...
    let sql = format!(
        "SELECT closedate, COALESCE(buydate, closedate), COALESCE(profitbtc, 0)
         FROM orders_rep WHERE {WHERE_PERIOD} ORDER BY closedate"
    );
...
fn top_trades(conn: &Connection, from: i64, to: i64, has_names: bool, best: bool) -> Vec<TopTrade> {
    ...
    let sql = format!(
        "SELECT o.closedate, COALESCE(o.coin,''), {name}, o.core_name,
         ...
         FROM orders_rep o WHERE {WHERE_PERIOD} AND o.profitbtc IS NOT NULL
...
fn groups(
    ...
    let sql = format!(
        "SELECT {key} AS k, COUNT(*), COALESCE(SUM(o.profitbtc),0),
         ...
         FROM orders_rep o WHERE {WHERE_PERIOD}
