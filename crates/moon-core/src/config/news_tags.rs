...
/// Tag name → palette colour key. Keys are the UI's fixed palette (`red`/`amber`/`green`/`blue`); an
/// unknown key resolves to neutral, so the file stays forward-compatible.
