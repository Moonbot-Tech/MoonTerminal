...
fn gaps_report_only_the_uncovered_stretches() {
    ...
    store.insert(key(), 120, 149, ticks(&[130]));
    assert_eq!(store.gaps(&key(), 100, 199), vec![(100, 119), (150, 199)]);
    store.insert(key(), 100, 119, Vec::new());
    store.insert(key(), 150, 199, ticks(&[160, 170]));
...
fn insert_clips_to_the_gaps_and_never_double_counts() {
    let mut store = TickTileStore::default();
    store.insert(key(), 100, 199, ticks(&[110, 150, 190]));
    // A wider re-fetch carrying the same prints again plus new edges.
    store.insert(key(), 50, 249, ticks(&[60, 110, 150, 190, 240]));
    assert_eq!(
...
fn coverage_run_follows_adjacency_not_the_hull() {
    let mut store = TickTileStore::default();
    store.insert(key(), 100, 199, Vec::new());
    store.insert(key(), 200, 299, Vec::new());
    store.insert(key(), 400, 449, Vec::new());
...
fn eviction_is_oldest_first_and_spares_the_current_insert() {
    ...
    store.insert(key(), 0, 999_999, ticks(&half));
    store.insert(
        key(),
        1_000_000,
        1_999_999,
    ...
    store.insert(key(), 2_000_000, 2_999_999, ticks(&third));
    ...
    store.insert(key(), 5_000_000, 5_999_999, ticks(&huge));
...
fn empty_tiles_are_bounded_by_count() {
    ...
    for i in 0..(TILE_STORE_MAX_TILES as i64 + 5) {
        store.insert(key(), i * 100, i * 100 + 99, Vec::new());
    }
    assert!(
...
fn one_insert_over_a_hull_files_every_gap_and_eviction_spares_them_all() {
    ...
    store.insert(key(), 120, 139, ticks(&[130]));
    store.insert(key(), 160, 179, ticks(&[170]));
    store.insert(key(), 100, 199, ticks(&[105, 150, 190]));
    assert!(
...
fn residual_is_empty_when_covered_and_whole_when_not() {
    ...
    store.insert(key(), 0, 299, ticks(&[5, 150, 250]));
...
fn residual_walks_a_split_slice_from_the_edge_that_touches_its_predecessors() {
    ...
    store.insert(key(), 230, 259, Vec::new());
    store.insert(key(), 40, 59, Vec::new());
    let residual = residual_plan(&p, &store, &key());
    assert_eq!(
        residual.slices,
        vec![
...
fn residual_keeps_a_backward_plan_descending() {
    ...
    store.insert(key(), 130, 159, Vec::new());
    let residual = residual_plan(&p, &store, &key());
    assert_eq!(
        residual.slices,
