...
pub(crate) const MB_MOONSHOT_ZONE_ALPHA: f32 = 0.09;
...
pub fn build_order_geometry(
    ...
    for ord in visible {
        ...
        if !closed {
            ...
            // Whether a MoonShot order fills its corridor is per chart tab
            // (`ChartGraphicsCfg::moonshot_zone`): the corridor spans the whole pane width and is
            // the loudest thing on a chart watching a MoonShot, so it is the one zone the user can
            // turn off. The panic-sell fill below is NOT covered by it — that one marks an order
            // about to be dumped and stays unconditional.
            if ord.is_moon_shot && graphics.moonshot_zone {
                push_zone(
