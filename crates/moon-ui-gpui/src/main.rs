...
struct Backend {
    ...
    /// Запросы «применить настройки свечей ко всем» из выносных окон
    /// (group, cfg, X-масштаб окна-источника) — дренится полоской вкладок группы.
    chart_candle_apply_all: Vec<(String, moon_core::market::CandleViewCfg, Option<f32>)>,
    /// Запрос синхронизации X-масштаба ([Shift+СКМ] на графике): (ОС-окно источника,
    /// px/ms). Ловит хозяин ТОГО ЖЕ окна (ChartTabs группы / DetachedChartHost) — масштаб
    /// применяется и сохраняется только для чартов этого окна.
    chart_x_sync: Option<(gpui::AnyWindowHandle, f32)>,
    /// Ревизия `chart_x_sync`: ++ на каждый жест (хозяева окон реагируют на рост).
