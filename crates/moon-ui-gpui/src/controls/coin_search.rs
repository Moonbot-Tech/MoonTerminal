...
const MOVER_VOL_REF: f64 = 100_000.0;
...
const SUGGEST_ROW_CAP: usize = 48;
...
fn mover_score(movement: f64, turnover_usd: f64) -> f64 {
...
fn turnover_usd(vol_24h: f64, rate: Option<f64>) -> Option<f64> {
    rate.map(|rate| {
...
fn neutralize_blind_provider(rows: &mut [Mover]) {
...
fn merge_ranked_heads(mut heads: Vec<Mover>, limit: usize) -> Vec<Mover> {
