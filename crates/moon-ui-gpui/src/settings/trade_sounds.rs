...
impl SettingsView {
    ...
    fn trade_sound_picker(
        ...
        let selected = crate::media::sound::names()
            .find(|name| *name == current.as_str())
            .unwrap_or("");
        let mut options = vec![(
            "",
            SharedString::from("mute"),
            SharedString::from(t!("trade_sounds.off").to_string()),
        )];
        options.extend(
            crate::media::sound::names()
                .map(|name| (name, SharedString::from(name), SharedString::from(name))),
        );
        let backend = self.backend.clone();
        let view = cx.entity().downgrade();
        let preview = current.clone();
        let preview_backend = self.backend.clone();
        let preview_disabled = selected.is_empty() || self.backend.read(cx).quiet_sleeping;
        ...
        field.child(
            h_flex()
                ...
                .child(
                    MoonDropdown::new(SharedString::from(format!(
                        "trade-sound-{}-{}-{open}",
                        id.code, id.dex
                    )))
                    .label(if current.is_empty() {
                        t!("trade_sounds.off").to_string()
                    } else {
                        current.clone()
                    })
                    .trigger_caret(true)
