...
impl AnalyticsView {
    pub(super) fn calendar_day(&self, hours: &[DayCell], p: MoonPalette, cx: &Context<Self>) -> AnyElement {
        ...
        let day_agg = |d0: i64| -> (f64, i64, i64) {
            hours
                ...
                .fold((0.0f64, 0i64, 0i64), |a, c| (a.0 + c.profit, a.1 + c.trades, a.2 + c.wins))
        };
        ...
        v_flex()
            ...
            .child(div().flex_1().w_full().min_h(px(0.0)).child(self.day_grid(&map, p, cx)))
            .into_any_element()
    }
    ...
    fn day_kpi(
        ...
        h_flex()
            ...
            .gap(design::ui_px(cx, 8.0))
            .items_stretch()
            .child(tile(p, cx, t!("analytics.cal.kpi_profit_day").to_string(), moon(sign_color(p, profit)), fmt_signed(profit), dp(profit, pp), false))
            .child(tile(p, cx, t!("analytics.kpi.trades").to_string(), moon(p.text), trades.to_string(), dp(trades as f64, pt as f64), false))
            .child(tile(p, cx, t!("analytics.cal.kpi_wins").to_string(), moon(p.green), wins.to_string(), dp(wins as f64, pw as f64), false))
            .child(tile(p, cx, t!("analytics.cal.kpi_losses").to_string(), moon(p.orange), losses.to_string(), dp(losses as f64, (pt - pw) as f64), true))
            .child(tile(p, cx, t!("analytics.kpi.winrate").to_string(), moon(p.text), format!("{wr:.1}%"), dp(wr, prev_wr), false))
    }

    fn day_grid(&self, map: &HashMap<i64, &DayCell>, p: MoonPalette, cx: &Context<Self>) -> AnyElement {
        ...
        while d <= bottom {
            ...
            let dt = date_of(d);
            let wd = wdays.get(dt.weekday().num_days_from_monday() as usize).cloned().unwrap_or_default();
            ...
            let gutter = h_flex()
                ...
                .cursor_pointer()
                .when(sel, |el| el.bg(moon_alpha(p.amber, 0.16)).border_1().border_color(moon_alpha(p.amber, 0.6)))
            ...
            let mut rowel = h_flex().flex_1().min_h_0().w_full().gap(cell_gap).child(gutter);
            for h in 0..24i64 {
                let hsec = d + h * 3600;
                rowel = rowel.child(self.hour_cell(hsec, map.get(&hsec).copied(), hour_max, sel, p, cx));
            }
            rows = rows.child(rowel);
            d += 86_400;
        }

        v_flex().size_full().gap(cell_gap).child(head).child(rows).into_any_element()
    }
    ...
    fn hour_cell(
        ...
        let bg = if sel_day { moon(p.panel_high) } else { moon(p.panel) };
        ...
        div()
            ...
            .child(div().absolute().inset_0().flex().items_center().justify_center().child(label))
            .into_any_element()
    }
}
