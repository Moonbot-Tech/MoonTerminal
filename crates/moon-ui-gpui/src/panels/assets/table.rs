...
impl AssetsView {
    ...
    /// cannot drift apart on what a click does: picking a transfer host asks that core for its
    /// transfer assets and rebuilds the cached snapshot, whichever shape the row was drawn in.
    ///
    /// Args:
    ///     agg: Balance aggregate of the core this row stands for.
    ///     selected: Core currently hosting the wallet containers, when one is in scope.
    ///     p: Active MoonUI palette, resolved once by the caller.
    ///     cx: Assets view context used to wire the click handler.
    ///
    /// Returns:
    ///     The row, styled as the active one when it is the current transfer host.
    fn wallet_core_row(
        &self,
        agg: &CoreAgg,
        selected: Option<CoreId>,
        p: MoonPalette,
        cx: &Context<Self>,
    ) -> AnyElement {
        let cid = agg.id;
        ...
        let mut item = h_flex()
            ...
            .on_click(cx.listener(move |this, _, window, cx| {
                ...
                if this.selected_core != Some(cid) {
                    ...
                    if let Err(error) = this.backend.read(cx).session.refresh_transfer_assets(cid) {
                        log::warn!("assets refresh failed for core {cid}: {error}");
    ...
    pub(super) fn bottom(
        ...
        // Display preferences of the core list below. Beside the refresh button rather than in the
        // panel toolbar: it configures this section, and the dock toolbar is shared with the
        // table-only Classic tab, which has no wallet section to configure.
        header = header.child(self.wallet_settings_popover(p, cx));
        if collapsed {
            return v_flex().w_full().flex_none().child(header);
        }

        // Left column: core names with free and total USDT balances. Grouped under exchange
        // headings, or one flat roster in core order when the section's gear turns grouping off.
        ...
        if self.group_by_venue(cx) {
            ...
            for (venue, members) in sections {
                ...
                for index in members {
                    list = list.child(self.wallet_core_row(&aggs[index], selected, p, cx));
                }
            }
        } else {
            for agg in aggs {
                list = list.child(self.wallet_core_row(agg, selected, p, cx));
            }
        }
