...
fn row(id: u64, address: Option<[u8; 4]>, port: u16, status: ConnStatus, sys: CoreSysStatus) -> CoreStatusRow {
    CoreStatusRow {
        id,
        name: format!("Core {id}"),
        status,
...
fn same_ip_with_different_ports_forms_one_server() {
    let rows = [
        row(11, Some([10, 20, 30, 40]), 3000, ConnStatus::Ready, CoreSysStatus::default()),
        row(12, Some([10, 20, 30, 40]), 4000, ConnStatus::Ready, CoreSysStatus::default()),
    ];

    let groups = aggregate_servers(&rows);

    assert_eq!(groups.len(), 1);
    assert_eq!(groups[0].key, ServerKey::Address(IpAddr::V4(Ipv4Addr::new(10, 20, 30, 40))));
    assert_eq!(groups[0].cores.iter().map(|core| core.id).collect::<Vec<_>>(), vec![11, 12]);
}
...
fn unknown_endpoints_remain_isolated() {
    let rows = [
        row(31, None, 0, ConnStatus::Disconnected, CoreSysStatus::default()),
        row(32, None, 0, ConnStatus::Disconnected, CoreSysStatus::default()),
    ];
...
fn servers_are_ordered_by_address() {
    let rows = [
        row(3, Some([10, 0, 0, 3]), 3000, ConnStatus::Ready, CoreSysStatus::default()),
        row(1, Some([10, 0, 0, 1]), 3000, ConnStatus::Ready, CoreSysStatus::default()),
        row(2, Some([10, 0, 0, 2]), 3000, ConnStatus::Ready, CoreSysStatus::default()),
    ];
...
fn cores_within_server_ordered_by_name() {
    ...
    let rows = [
        row(74, address, 3000, ConnStatus::Ready, CoreSysStatus::default()),
        row(72, address, 3001, ConnStatus::Ready, CoreSysStatus::default()),
        row(71, address, 3002, ConnStatus::Ready, CoreSysStatus::default()),
        row(73, address, 3003, ConnStatus::Ready, CoreSysStatus::default()),
    ];

    let groups = aggregate_servers(&rows);

    assert_eq!(groups[0].cores.iter().map(|core| core.id).collect::<Vec<_>>(), vec![71, 72, 73, 74]);
}
...
fn partial_readiness_is_degraded() {
    let rows = [
        row(41, Some([203, 0, 113, 5]), 3000, ConnStatus::Ready, CoreSysStatus::default()),
        row(42, Some([203, 0, 113, 5]), 3001, ConnStatus::Failed("offline".to_string()), CoreSysStatus::default()),
    ];
