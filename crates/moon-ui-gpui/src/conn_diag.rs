...
use moon_core::feed::{Diagnosis, FailureClass};
use moon_core::session::CoreInitStep;
use rust_i18n::t;
...
pub(crate) fn fault_facts(d: &Diagnosis) -> Vec<FaultFact> {
    let mut out = Vec::new();
...
pub(crate) fn fault_line(d: &Diagnosis) -> String {
    let mut why = reason(&d.class);
    if d.retrying {
        why = format!("{why} ({})", t!("core_status.fault.retrying"));
    }
    format!("{why} \u{2014} {}", next_step(&d.class))
}
