...
pub(crate) struct Shell {
    ...
    /// Which manual-strategy quick-select slot has its assign menu open, as `(core, slot)`.
    ///
    /// Addressed by CORE rather than by group: the buttons belong to one core's slots, and the
