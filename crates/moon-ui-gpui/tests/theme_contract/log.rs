...
#[test]
fn side_badge_delegates_to_the_shared_soft_density_tag_builder() {
    ...
    assert!(
        tag_badge.contains("MoonBadge::new(text)")
            ...
            && !tag_badge.contains(".size(")
            && tag_badge.contains(".bg_color(color)")
            && tag_badge.contains(".text_color(color)"),
        "tag_badge must retain the shared Soft density-default builder with color in both fill and text"
    );
}
