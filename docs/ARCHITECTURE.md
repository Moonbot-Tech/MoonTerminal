...
- **Живой чарт** — стиль «Сделки: Метки / Линии Moonbot» (`ChartGraphicsCfg::trade_history_style`,
  per-tab, дефолт по виду через ⧉). В режиме линий метка сделки уступает место линиям только
  когда архив их отдал — старая сделка без трасс остаётся стрелками; движок сам ранжирует
