...
пользователем: `CoreSortMode` (`moon-core/src/config/servers.rs`) — свой порядок
(перетаскиванием), по имени или по добавлению; хранится в `settings.toml`.
...
- Смена режима — презентационная: она нейтрализована в `AppConfig::structural_sig`, реконнекта
  ядер не вызывает. Ручной РЕОРДЕР меняет порядок `servers` и потому остаётся структурным.
